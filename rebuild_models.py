...
import pandas as pd
import numpy as np
import pickle # <-- NEW: Using pickle instead of joblib
...
print("--- Starting FINAL MODEL RE-SAVING FIX with PICKLE ---")

# --- 1. Data Preparation (Must match Step 4) ---
df = pd.read_csv('emi_prediction_dataset.csv', low_memory=False) # Added low_memory=False
epsilon = 1e-6

# Cleaning & Type Conversion (Must be run again)
...
# --- 2. Build and Save XGBoost Pipelines using PICKLE ---
...
# Save with PICKLE (binary write mode)
with open('best_emi_classifier_pipeline.pkl', 'wb') as file:
    pickle.dump(final_cls_pipeline, file)
print("✅ Classification Model (best_emi_classifier_pipeline.pkl) successfully rebuilt and saved with PICKLE.")
...
# Save with PICKLE (binary write mode)
with open('best_emi_regressor_pipeline.pkl', 'wb') as file:
    pickle.dump(final_reg_pipeline, file)
print("✅ Regression Model (best_emi_regressor_pipeline.pkl) successfully rebuilt and saved with PICKLE.")
...
>>>>>>> 0d1692b800d57f0d03a278f74a3e90f793a23a7e
print("--- PICKLE FIX COMPLETE. ---")